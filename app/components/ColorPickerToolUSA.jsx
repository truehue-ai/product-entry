...
export default function ColorPickerToolUSA({ initialBrand = "", initialProduct = "" }) {
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/upload-shades-usa', {
      method: 'POST',
    ...
    if (Object.keys(linkMap).length > 0) {
      await fetch('/api/upload-links-json-usa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ brand, product, links: linkMap }) });
    }
    const priceMap = {};
    filteredShades.forEach(s => { if (s.price) priceMap[s.name] = s.price; });
    if (Object.keys(priceMap).length > 0) {
      await fetch('/api/upload-price-json-usa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ brand, product, price: priceMap }) });
    }
    const typeMap = {};
    filteredShades.forEach(s => { if (category) typeMap[s.name] = category; });
    if (Object.keys(typeMap).length > 0) {
      await fetch('/api/upload-type-json-usa', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ brand, product, type: typeMap }) });
    }
  ...
  const handleMakeLive = async () => {
    ...
    if (!confirmed) return;
    try {
      const res = await fetch('/api/update-all-data-usa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSuggestionClick = async (item) => {
    ...
    try {
      const r = await fetch(
        `/api/logs/get?brand=${encodeURIComponent(item.brand)}&product=${encodeURIComponent(item.product)}&usa=0`,
        { cache: "no-store" }
      );
      const j = await r.json();
      if (j?.shades) {
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14, overflow: "visible" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <input
                  className="th-input"
                  style={GL.input}
                  placeholder="e.g. Sheer Glow"
                  value={product}
                  onChange={(e) => { setProduct(e.target.value); setShowSuggestions(false); }}
                ...
                {showSuggestions && (
                  <div style={{
                    ...
                    borderRadius: 11,
                    boxShadow: "0 8px 32px rgba(171,31,16,0.13)",
                    marginTop: 4,
                    maxHeight: 220,
                    overflowY: "auto",
                  }}>
                    <div style={{ padding: "8px 12px 6px", fontSize: 11, fontWeight: 700, color: "#9b4a42", letterSpacing: "0.06em", textTransform: "uppercase", borderBottom: "1px solid rgba(171,31,16,0.07)" }}>
                      From India catalogue {suggestionsLoading ? "…" : `(${brandSuggestions.length})`}
                    </div>
                    {brandSuggestions.map((it) => (
                      <div
                        key={`${it.brand}:::${it.product}`}
                        onMouseDown={() => handleSuggestionClick(it)}
                        style={{
                          padding: "11px 14px",
                          fontSize: 14,
                          fontWeight: 500,
                        ...
                        onMouseEnter={(e) => e.currentTarget.style.background = "rgba(171,31,16,0.06)"}
                        onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}
                      >
                        {it.product}
                        <span style={{ fontSize: 11, color: "#9b4a42", marginLeft: 8 }}>tap to import</span>
                      </div>
                    ))}
                  </div>
                )}
